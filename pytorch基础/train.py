...
def train():
    ...
    # 构建DataLoder,将train_data中所有图片路径及标签分为对应的一个个batch
    ...
    # 设置学习率下降策略：学习率下降间隔数，若为10，则会在10、20、30…个step（step指epoch），将学习率调整为lr*gamm
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(train_loader):
            '''
            i: batch_index
            data: list:2  两个tensor，一个是batch张图片数据，一个是label
            '''
            # forward
            inputs, labels = data
            outputs = net(inputs)
